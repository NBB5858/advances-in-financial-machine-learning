...
class CLA:
        ...
        while True:
            ...
            if len(f) > 1:
                ...
                for j, i in enumerate(f):
                    result = self.computeLambda(covarF_inv, covarFB, meanF, wB, j, [self.lB[i], self.uB[i]])
                    if result is None:
                        continue
                    l, bi = result
                    if l is not None and l > l_in:
            ...
            if len(f) < self.mean.shape[0]:
                ...
                for i in b:
                    ...
                    covarF_inv = np.linalg.inv(covarF)
                    result = self.computeLambda(covarF_inv, covarFB, meanF, wB, meanF.shape[0] - 1, self.w[-1][i])
                    if result is None:
                        continue
                    l, bi = result
            ...
            if l_in < 0 and l_out < 0:
                ...
                # 4) Decide which lambda to apply
                if l_in > l_out:
                    self.l.append(l_in)
                    f.remove(i_in)
                    covarF, covarFB, meanF, wB = self.getMatrices(f)

                    w[i_in] = bi_in  # Full vector assignment

                    # ✅ Correct mapping from global index to local wB index
                    B = self.getB(f)
                    if i_in in B:
                        idx_in_B = B.index(i_in)
                        wB[idx_in_B] = bi_in
                else:
                    self.l.append(l_out)
                    f.append(i_out)
    ...
    def computeLambda(self,covarF_inv,covarFB,meanF,wB,i,bi):
        ...
        if wB is None:
            ...
            ret_val = (((1-l1+l2)*c4[i]-c1*(bi+l3[i]))/c).item()
